class TokenType:
    """Tipos de token del lexer regex."""
...
class Token:
    """Token con tipo y valor (char, clase, etc.)."""
...
class Lexer:
    """Tokeniza una expresion regular (. * + ? | [] escapes)."""
    ...
    def current(self):
        """Caracter actual o None al final."""
        if self.pos < len(self.pattern):
            return self.pattern[self.pos]
        return None

    def advance(self):
        """Consume y devuelve el caracter actual."""
        ch = self.pattern[self.pos]
        self.pos += 1
        return ch

    def _expand_escape(self, ch: str):
        """Expande secuencia de escape; ValueError si no es valida."""
    ...
    def _parse_char_class(self):
        """Parsea [...] y retorna Token CHAR_CLASS."""
        chars = set()
        negated = False
    ...
    def tokenize(self):
        """Devuelve la lista de tokens terminada en END."""
        self.tokens = []
        self.pos = 0
